...
/*   Updated: 2024/03/11 12:16:26 by tauer            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hooking(t_data *data)
{
	...
	// mlx_hook(data->win_num, 17, 0L, &close_win, data);
	mlx_hook(data->win, 2, 1L << 0, &true_key, data);
	// mlx_hook(data->win_num, 2, 1L << 0, &true_key, data);
	mlx_hook(data->win, 3, 1L << 1, &false_key, data);
	// mlx_hook(data->win_num, 3, 1L << 1, &false_key, data);
