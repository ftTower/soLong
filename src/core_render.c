...
/*   Updated: 2024/03/01 20:40:23 by tauer            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render(t_data *data)
{
	...
	while (data->map[x] != NULL)
	{
		...
		while (data->map[x][y])
		{
			if (data->map[x][y] == 'X')
			{
				put_img_to_img(data->base_image, data->textures.empty, x_pos,
					y_pos);
			}
			...
			else if (data->map[x][y] == 't')
			{
				put_img_to_img(data->base_image,
					data->textures.current_screen_outdoor, x_pos, y_pos);
			}
			else if (data->map[x][y] == 'x')
			{
				put_img_to_img(data->base_image, data->textures.panel_wall,
					x_pos, y_pos);
			}
			else if (data->map[x][y] == 'y')
			{
				put_img_to_img(data->base_image,
					data->textures.current_screen_indoor, x_pos, y_pos);
			}
			...
			else if (data->map[x][y] == 'q')
			{
				if (check_empty(data, x, y + 1) || check_walls(data, x, y + 1))
					...
					put_img_to_img(data->base_image, data->textures.empty,
						x_pos, y_pos);
				put_img_to_img(data->base_image,
					data->textures.right_top_corner_up, x_pos, y_pos);
			}
